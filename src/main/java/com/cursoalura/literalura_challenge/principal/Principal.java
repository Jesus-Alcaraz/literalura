...
public class Principal {
    ...
    private void buscarTitulosPorNombre() {
        System.out.println("Por favor escriba el nombre del autor");
    ...
    private void mostrarMenuDeIdiomas() {
        ...
        System.out.println("""
                ...
                en: Inglés
                """);
        System.out.println("En que idioma desea buscar los libros:");
        String entrada = teclado.nextLine();
        List<Titulo>  titulosTemp = repositorioAutor.listaTitulosPorIdioma(entrada);
        if (titulosTemp.isEmpty()){
            System.out.println("No encontrado, por favor intente de nuevo");
        }
        titulosTemp.forEach(System.out::println);
    }
    ...
    private void mostrarLiveYearCheck() {
        System.out.println("Por favor elija el año en vida del autor(es) que desea encontrar");
        Integer year = teclado.nextInt();
        teclado.nextLine();

        List<Autor>  autorsTemp = repositorioAutor.liveYearCheck(year);
        if (autorsTemp.isEmpty()){
            System.out.println("No encontrado, por favor intente de nuevo la busqueda");
        }
    ...
    private void buscarLibroPorTitulo() throws IOException, InterruptedException{
        ...
        if (titulo.isPresent()) {
            ...
            if (repositorioAutor.buscarTituloPorNombre(tituloTemp.getTitulo()).isPresent()) {
                System.out.println("Este liro ya se encuentra registrado, no se puede registrar un libro más de una vez");
